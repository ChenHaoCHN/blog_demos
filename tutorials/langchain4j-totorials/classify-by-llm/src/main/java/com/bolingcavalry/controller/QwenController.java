...
public class QwenController {
    ...
    @PostMapping("/output/analyzesentimentof")
    public ResponseEntity<Response> analyzeSentimentOf(@RequestBody PromptRequest request) {
        ...
        try {
            String response = qwenService.analyzeSentimentOf(request.getPrompt());
    ...
    @PostMapping("/output/ispositive")
    public ResponseEntity<Response> isPositive(@RequestBody PromptRequest request) {
        ResponseEntity<Response> checkRlt = check(request);
        if (checkRlt != null) {
            return checkRlt;
        }
        ...
        try {
            String response = qwenService.isPositive(request.getPrompt());
            return ResponseEntity.ok(new Response(response));
        } catch (Exception e) {
            // 捕获异常并返回错误信息
            return ResponseEntity.status(500).body(new Response("请求处理失败: " + e.getMessage()));
